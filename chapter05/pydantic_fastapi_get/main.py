...
class User(BaseModel):
    ...
    age: int = Field(
        ..., title="年龄", description="年龄需要大于18岁", ge=18, example=12
    )
    password_old: str = Field(
        ..., title="旧密码", description="密码需要长度大于6", gl=6, example=6
    )
    password_new: str = Field(
        ..., title="新密码", description="密码需要长度大于6", gl=6, example=6
    )

    @root_validator
    def check_passwords(cls, values):
        password_old, password_new = values.get("password_old"), values.get(
            "password_new"
        )
        # 新旧号码的确认匹配处理
...
async def file_get(user: FileGet = Depends()):
    return {"username": user.username, "filenme": user.file.filename}
