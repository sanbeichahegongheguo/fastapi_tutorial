        ...
        if not (authorization and scheme and credentials):
            if self.auto_error:
                if self.realm and self.qop:
                    ...
                    self.unauthorized_headers = {
                        "WWW-Authenticate": f'Digest realm="{self.realm}",'
                    }
                raise HTTPException(
...
def default_verify_password(password, credentials, request: Request):
    ...
    if datas:
        ha1 = generate_ha1(
            username=datas.get("username"), realm=realm, password=password
        )
...
@app.get("/digest")
async def digest(
    request: Request, auth: HTTPAuthorizationCredentials = Depends(security)
):
    # print(credentials)
    if default_verify_password(
        password="123456", credentials=auth.credentials, request=request
    ):
