...
async def get_current_active_user(
    current_user: User = Security(get_current_user, scopes=["get_admin_info"])
):
...
async def access_token(client_id: str, client_secret: str, code: str):
    ...
    refresh_access_token_expires = timedelta(
        minutes=REFRRSH_ACCESS_TOKEN_EXPIRE_MINUTES
    )
    # 刷新refresh_token
...
async def authorizationUrl(
    ...
    return RedirectResponse(
        url=f"/oauth2/authorize/user/agent?redirect_uri={redirect_uri}"
    )
...
async def user_agent(
    *, credentials: HTTPBasicCredentials = Depends(security), redirect_uri: str
):
...
async def get_admin_info(
    current_user: User = Security(get_current_active_user, scopes=["get_admin_info"])
):
    return current_user


@app.get("/api/v1/del_admin_info", response_model=User)
async def del_admin_info(
    current_user: User = Security(get_current_active_user, scopes=["del_admin_info"])
):
    return current_user
...
async def get_user_info(
    current_user: User = Security(get_current_active_user, scopes=["get_user_info"])
):
    return current_user
...
async def get_user_role(
    current_user: User = Security(get_current_active_user, scopes=["get_user_role"])
):
    return current_user
...
async def get_user_role(
    current_user: User = Security(
        get_current_active_user, scopes=["get_user_permission"]
    )
):
    return current_user
