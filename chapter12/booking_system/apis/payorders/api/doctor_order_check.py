...
async def callbadk(
    forms: SubscribeOrderCheckForm = Depends(),
    ...
    doctor_nsnum_info_result = (
        await PayOrderServeries.get_order_info_dno_orderid_visituopenid_state(
            db_session, forms.dno, forms.visit_uopenid, forms.orderid
        )
    )
    ...
    return (
        Success(
            api_code=200,
            result={"status": doctor_nsnum_info_result.statue},
            message="查询成功",
        )
        if doctor_nsnum_info_result
        else Fail(api_code=200, result=None, message="无排班记录信息,或已过有效期")
    )
