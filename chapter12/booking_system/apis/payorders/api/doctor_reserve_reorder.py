...
async def callbadk(
    ...
    if doctor_order_info_result.statue == 4:
        return Fail(
            api_code=200, result=None, message="订单已超时未支付！建议取消重新下单！"
        )
    ...
    if doctor_order_info_result.statue != 1:
        return Fail(
            api_code=200,
            result=None,
            message="订单信息状态异常信息！无法继续支付！建议取消重新下单！",
        )

    # 查询当前预约时段
    nsindex = doctor_order_info_result.nsindex
    # 查询排班信息
    doctor_nsnum_info_result = (
        await PayOrderServeries.get_doctor_scheduling_info_info_order(
            db_session, dno=forms.dno, nsindex=nsindex
        )
    )
    ...
    order_info = {
        ...
        "nsindex": nsindex,
        "orderid": (
            doctor_order_info_result.orderid
            if doctor_order_info_result.orderid
            else None
        ),
    ...
    try:
        ...
        visittime = doctor_order_info_result.visittime
        # =================
        doctor_info_result = await PayOrderServeries.get_doctor_info(
            db_session, dno=forms.dno
        )
        ...
        return Fail(
            api_code=200,
            result=None,
            message=f"微信支付配置服务异常，请稍后重试！！错误提示{wcpayex.errmsg}",
        )
    except Exception:
        import traceback
