...
class PayOrderServeries:
    ...
    @staticmethod
    async def get_order_info_dno_orderid_visituopenid_state(
        async_session: AsyncSession, dno, visit_uopenid, orderid
    ) -> DoctorSubscribeinfo:
    ...
    @staticmethod
    async def get_doctor_scheduling_info_info_order(
        async_session: AsyncSession, dno, nsindex
    ) -> DoctorScheduling:
        query = select(
    ...
    @staticmethod
    async def get_order_info_byvisit_uopenid_state(
    ...
    @staticmethod
    async def updata_order_info_byorder_dno(
        async_session: AsyncSession, dno, orderid, visit_uopenid, **updata
    ):
        # # 判断是否存在
        #  stmt = select(User).where(User.name == 'sandy').exists()
