...
async def callbadk(
    ...
    if doctor_nsnum_info_result.statue == 3:
        ...
        return Fail(
            api_code=200, result=None, message="订单已操作过取消！请勿重复操作！"
        )
    ...
    if doctor_nsnum_info_result.statue == 5:
        ...
        # 更新没有支付的成功的订单的，状态！
    isok = await Serveries.updata_order_info_byorder_dno_olny(
        db_session,
        dno=forms.dno,
    ...
    return (
        Success(api_code=200, result=None, message="订单取消成功")
        if isok
        else Fail(api_code=200, result=None, message="订单取消失败")
    )
