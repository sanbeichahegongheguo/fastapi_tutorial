...
async def callbadk(
    forms: SubscribeOrderCheckForm,
    db_session: AsyncSession = Depends(depends_get_db_session),
):
    # 检测订单的状态
    doctor_nsnum_info_result = await Serveries.get_order_info_byorder_dno_state(
        db_session, dno=forms.dno, orderid=forms.orderid
    )
    ...
    if doctor_nsnum_info_result.statue == 5:
        pass
        return Fail(
            api_code=200, result=None, message="该订单处于申请退款状态,请勿重复操作！"
        )
    ...
    if doctor_nsnum_info_result.statue == 3:
        ...
        # 订单状态（1:订单就绪，还没支付 2：已支付成功 3：取消订单 4：超时未支付订单）
        return Fail(
            api_code=200, result=None, message="该订单已操作过取消！请勿重复操作！"
        )
    ...
    if doctor_nsnum_info_result.statue == 2:
        ...
        datiem = currday_time_info_tochane_datetime(
            doctor_nsnum_info_result.visitday
        ) - datetime.timedelta(days=1)
        subscribe_times = datetime.datetime.strptime(
            f"{str(datiem).replace('00:00:00', '')}23:59:59", "%Y-%m-%d %H:%M:%S"
        )
        ...
        if todaydate >= subscribe_times:
            return Fail(
                api_code=200,
                result=None,
                message="非常抱歉，您已超过申请退款约定时间，暂无法给你申请退款处理！",
            )

    isok = Serveries.updata_order_info_byorder_dno_olny(
        db_session,
        dno=forms.dno,
        orderid=forms.orderid,
        visit_uopenid=forms.visit_uopenid,
        updata={"statue": 5, "refund_statue": 1, "apply_refund_time": "now()"},
    )
    return (
        Success(api_code=200, result=None, message="申请退款成功！已提交审核！")
        if isok
        else Fail(api_code=200, result=None, message="申请退款失败！")
    )
