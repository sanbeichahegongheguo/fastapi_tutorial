...
try:
    app.mount(
        "/static", StaticFiles(directory=f"{pathlib.Path.cwd()}/static"), name="static"
    )
except:
    pass
...
async def startup_callback_init_data():
    ...
    await async_rabbit_client.make_exchange_declare(
        exchange_name=order_dead_letter_exchange_name,
        exchange_type=order_dead_letter_exchange_type,
    )
    await async_rabbit_client.make_queue_declare(
        queue_name=order_dead_letter_queue_name
    )

    await async_rabbit_client.make_queue_bind(
        exchange_name=order_dead_letter_exchange_name,
    ...
    await async_rabbit_client.make_exchange_declare(
        exchange_name=order_exchange_name, exchange_type=order_exchange_type
    )
    await async_rabbit_client.make_queue_declare(
        queue_name=order_queue_name, arguments=order_queue_arguments
    )
    await async_rabbit_client.make_queue_bind(
        exchange_name=order_exchange_name, routing_key=order_routing_key
    )


async_rabbit_client.init_app(
    app=app, rabbitconf=get_settings(), startup_callback=startup_callback_init_data
)
...
def startup_callback_init_data_sync():
    ...
    sync_rabbit_client.make_queue_declare(
        queue_name=order_queue_name, arguments=order_queue_arguments
    )
    sync_rabbit_client.make_queue_bind(
