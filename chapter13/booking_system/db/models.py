...
class DoctorScheduling(Base):
    ...
    id = Column(
        Integer,
        primary_key=True,
        server_default=text("nextval('doctor_scheduling_id_seq'::regclass)"),
        comment="主键Id",
    )
    dno = Column(
        Text,
    ...
    nsindex = Column(
        Text, unique=True, server_default=text("''::text"), comment="号源编号"
    )
    dnotime = Column(Date, comment="排班日期，年-月-日")
    tiemampmstr = Column(
        Text, server_default=text("''::text"), comment="号源时段字符串显示"
    )
    ampm = Column(
        Text, server_default=text("''::text"), comment="医生工作日：上午 还是 下午"
    )
    create_time = Column(
        TIMESTAMP(precision=0), server_default=text("now()"), comment="创建时间"
    )
    enable = Column(Integer, comment="是否可用（1：是 0 否）")
    tiempm = Column(
        TIMESTAMP(precision=6), comment="医生工作日：号源时段(年-月-日 时：分)"
    )
...
class DoctorSubscribeinfo(Base):
    ...
    id = Column(
        Integer,
        primary_key=True,
        server_default=text("nextval('doctor_subscribeinfo_id_seq'::regclass)"),
        comment="主键Id",
    )
    dno = Column(
        Text,
    ...
    orderid = Column(
        Text, index=True, server_default=text("''::text"), comment="订单编号"
    )
    nsindex = Column(Text, server_default=text("''::text"), comment="订单编号")
    statue = Column(
        Integer,
        server_default=text("1"),
        comment="订单状态（1:订单就绪，还没支付 2：已支付成功 3：取消订单",
    )
    ...
    visit_uopenid = Column(
        Text, server_default=text("''::text"), comment="就诊人微信ID"
    )
    visit_uname = Column(Text, server_default=text("''::text"), comment="就诊人姓名")
    visit_uphone = Column(
        Text, server_default=text("''::text"), comment="就诊人联系电话"
    )
...
class Doctorinfo(Base):
    ...
    id = Column(
        Integer,
        primary_key=True,
        server_default=text("nextval('doctorinfo_id_seq'::regclass)"),
        comment="主键Id",
    )
    dno = Column(
        Text,
...
class Hospitalinfo(Base):
    ...
    id = Column(
        Integer,
        primary_key=True,
        server_default=text("nextval('hospitalinfo_id_seq'::regclass)"),
        comment="主键Id",
    )
    name = Column(Text, server_default=text("''::text"), comment="医院名称")
    describe = Column(Text, server_default=text("''::text"), comment="医院描述")
    describeimages = Column(
        Text, server_default=text("''::text"), comment="describeimages"
    )
    create_time = Column(
        TIMESTAMP(precision=0), server_default=text("now()"), comment="创建时间"
    )
