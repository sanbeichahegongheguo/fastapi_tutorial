...
from exts.wechatpy.client import WeChatComponentClient
from exts.wechatpy.constants import WeChatErrorCode
from exts.wechatpy.crypto import WeChatCrypto
...
from exts.wechatpy.fields import DateTimeField, StringField
from exts.wechatpy.messages import MessageMetaClass
from exts.wechatpy.session.memorystorage import MemoryStorage
from exts.wechatpy.utils import get_querystring, json, to_binary, to_text, ObjectDict
...
class BaseWeChatComponent(object):
    ...
    def __init__(
        ...
        if isinstance(session, six.string_types):
            ...
            from exts.wechatpy.session.shovestorage import ShoveStorage
    ...
    def access_token(self):
        ...
        if access_token:
            if not self.expires_at:
                # order provided access_token, just return it
                return access_token
...
class WeChatComponent(BaseWeChatComponent):
    ...
    def get_client_by_authorization_code(self, authorization_code):
        ...
        """
        warnings.warn(
            "`get_client_by_authorization_code` method of `WeChatComponent` is deprecated,"
        ...
        return WeChatComponentClient(
            authorizer_appid, self, access_token, refresh_token, session=self.session
        )
