...
if __name__ == "__main__":
    ...
    with sync_context_get_db() as session:
        ed_user = Hospitalinfo(
            name="ed", describe="Ed Jones", describeimages="edsnickname"
        )
        session.add(ed_user)

        _result: DoctorSubscribeinfo = (
            session.query(DoctorSubscribeinfo)
            .filter(
                and_(
                    DoctorSubscribeinfo.dno == "10001",
                    DoctorSubscribeinfo.visit_uopenid == "orE7I59UwXdWzfSK9QGK2fHGtPZ8",
                    DoctorSubscribeinfo.orderid == "2207121523229771546",
                )
            )
            .one_or_none()
        )
